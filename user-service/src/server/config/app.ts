...
// please keep requestId middleware before auth so that a requestid is generated
// for call made to auth service and the same is passed ahead
...
export const startServer = (cb: Function, port?: number) => {
  ...
  server = (server || app).listen(port, () => {
    logger.info('Calorie Tracking Service: Listening on port ' + port + '!');
  });
  cb(server);
};
